...
class IndicatorsByStatus extends LocalizeMixin(ReduxConnectedElement) {
  public static get template() {
    return html`
      ...
      <paper-card class="widget-container">
        ...
        <div class="app-grid">
          <div class="item">
            <google-chart type="pie" options="[[chartOptions]]" cols="[[chartCols]]" rows="[[chartRows]]">
            </google-chart>
          </div>
          <div class="item item-wide">
            <chart-legend rows="[[chartRows]]" colors="[[colors]]"> </chart-legend>
          </div>
        </div>
  ...
  @property({type: Array})
  colors = ['#029a53', '#2bb0f2', '#d8d8d8', '#fecc02', '#273d48'];

  @property({type: Array})
  labels = ['Met', 'On Track', 'No Progress', 'Constrained', 'No Status'];

  @property({
    type: Number,
    computed: 'getReduxStateValue(rootState.clusterDashboardData.data.num_of_met_indicator_reports)'
  })
  met_count!: number;

  @property({
    type: Number,
    computed: 'getReduxStateValue(rootState.clusterDashboardData.data.num_of_on_track_indicator_reports)'
  })
  on_track_count!: number;

  @property({
    type: Number,
    computed: 'getReduxStateValue(rootState.clusterDashboardData.data.num_of_no_progress_indicator_reports)'
  })
  no_progress_count!: number;

  @property({
    type: Number,
    computed: 'getReduxStateValue(rootState.clusterDashboardData.data.num_of_constrained_indicator_reports)'
  })
  constrained_count!: number;

  @property({
    type: Number,
    computed: 'getReduxStateValue(rootState.clusterDashboardData.data.num_of_no_status_indicator_reports)'
  })
  ...
  @property({
    ...
    computed:
      ...
      ['met_count', 'on_track_count', 'no_progress_count', 'constrained_count', 'no_status_count'].join(', ') +
      ')'
  })
  chartRows!: any[];
  ...
  _computeChartRows(labels: any[], ...args: any[]) {
    ...
    return data.map(function (count, index) {
      return [localizedLabels[index], count];
    });
  }
}
