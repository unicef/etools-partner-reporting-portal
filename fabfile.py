...
def ssh(service):
    ...
    :param service: ['backend', 'polymer', 'proxy', 'db', 'id-management-frontend']
    """
    assert service in ['backend', 'polymer', 'proxy', 'db', 'id-management-frontend'], "%s is unrecognized service"
    if service == 'polymer':
...
def restart(service):
    ...
    :param service: ['backend', 'polymer', 'proxy', 'db', 'id-management-frontend']
    """
    assert service in ['backend', 'polymer', 'proxy', 'db', 'beater-prp', 'worker-prp', 'id-management-frontend'], "%s is unrecognized service"
...
def fixtures():
    ...
    """
    local('docker-compose exec backend python manage.py loaddata sites')
    local('docker-compose exec backend python manage.py loaddata reporting_entities')
    local('docker-compose exec backend python manage.py loaddata periodic_tasks')
...
def fake_data(quantity=1):
    ...
    """
    local('docker-compose exec backend python manage.py generate_fake_data --quantity %d --clean_before' % (int(quantity)))
...
def real_data(area=False):
    ...
    """
    local('docker-compose exec backend python manage.py generate_real_data --clean_before %s' % ("--area %s --fast" % area if area else ""))
...
def generate_reports():
    ...
    """
    local('docker-compose exec backend python manage.py generate_reports')
...
def sync_ocha_partners(area=False):
    ...
    Generate reports based on current data
    """
    local('docker-compose exec backend python manage.py sync_ocha_partners %s' % ("--country %s" % area if area else ""))
...
def tests(test_path=''):
    ...
    Run backend tests.
    """
    local('docker-compose exec backend python manage.py test {} --settings=etools_prp.config.settings.test --parallel --noinput'.format(test_path))
...
def update_real_fixtures(area=False):
    ...
    """
    local('docker-compose exec backend python manage.py generate_real_data %s --update' % ("--area %s --fast" % area if area else ""))
