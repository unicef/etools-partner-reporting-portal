...
class Country(TenantMixin):
    ...
    # TODO: rename the related name as it's inappropriate
    # for relating offices to countries.. should be office_countries
    offices = models.ManyToManyField('Office', related_name='offices')
    sections = models.ManyToManyField('Section', related_name='sections')

    threshold_tre_usd = models.DecimalField(
        max_digits=20, decimal_places=4, default=None, null=True)
    threshold_tae_usd = models.DecimalField(
        max_digits=20, decimal_places=4, default=None, null=True)
...
class CountryOfficeManager(models.Manager):
    def get_queryset(self):
        if hasattr(connection.tenant, 'id') and \
            connection.tenant.schema_name != 'public':
            return super(CountryOfficeManager, self).get_queryset().filter(offices=connection.tenant)

        else:
            # This only gets called on initialization because
            # FakeTenant does not have the model attrs
...
class UserProfileManager(models.Manager):
    def get_queryset(self):
        ...
        .get_queryset().select_related('country')


class UserProfile(models.Model):
    """
    Represents a user profile that can have access to many Countries
    but to one active Country at a time
    ...
    installation_id = models.CharField(
        max_length=50, null=True, blank=True, verbose_name='Device ID')
    ...
    supervisor = models.ForeignKey(
        User, related_name='supervisee',
        on_delete=models.SET_NULL, blank=True, null=True)
    oic = models.ForeignKey(
        User, blank=True,
        on_delete=models.SET_NULL, null=True)  # related oic_set
    ...
    # vision_roles = ArrayField(
    #   models.CharField(max_length=20, blank=True,
    #       choices=VISION_ROLES), blank=True, null=True)
    ...
    def custom_update_user(cls, sender, attributes, user_modified, **kwargs):
        ...
        if sender.profile.country_override:
            ...
            try:
                new_country = Country.objects.get(
                    business_area_code=adfs_country[0])
