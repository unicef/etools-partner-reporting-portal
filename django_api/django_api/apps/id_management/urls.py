...
from account.views import UserListCreateAPIView, UserDeactivateAPIView, UserActivateAPIView
...
urlpatterns = [
    ...
    url(r'^users/(?P<pk>\d+)/deactivate/$', UserDeactivateAPIView.as_view(), name='user-deactivate'),
    url(r'^users/(?P<pk>\d+)/activate/$', UserActivateAPIView.as_view(), name='user-activate'),
