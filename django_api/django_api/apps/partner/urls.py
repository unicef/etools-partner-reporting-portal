...
urlpatterns = [
    ...
    url(r'^(?P<response_plan_id>\d+)/partner-project-list/imo/(?P<partner_id>\d+)/$',
    ...
    url(r'^(?P<response_plan_id>\d+)/create-partner-activity/imo/(?P<partner_id>\d+)/(?P<create_mode>\w+)$',
        PartnerActivityCreateAPIView.as_view(),
        name="imo-partner-activity-create"),
