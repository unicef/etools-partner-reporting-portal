...
class IndicatorsXLSXExporter:
    ...
    def generate_sheet(self, disaggregation_types):
        ...
        self.sheet.title = ",".join(
            [dt.name for dt in disaggregation_types]) if disaggregation_types else "None"
        ...
        for idx, dt in enumerate(disaggregation_types):
            ...
            self.sheet.cell(
                row=4,
                column=DISAGGREGATION_COLUMN_START + idx).value = "#indicator+type+" + dt.name
            self.sheet.cell(
                row=4,
                column=DISAGGREGATION_COLUMN_START +
                idx).alignment = Alignment(
        ...
        for disaggregation_type in disaggregation_types:
            disaggregation_values_base = DisaggregationValue.objects.filter(
                disaggregation=disaggregation_type).order_by('value')
        ...
        for idx, dvs in enumerate(disaggregation_values):
            self.sheet.cell(row=1, column=DISAGGREGATION_COLUMN_START +
                            len(disaggregation_types) +
                            idx).value = " + ".join([dv.value for dv in dvs])
            self.sheet.cell(
                row=1,
                column=DISAGGREGATION_COLUMN_START +
                len(disaggregation_types) +
            ...
            self.sheet.cell(
                ...
                idx).font = Font(
                bold=True)
            self.sheet.cell(row=2, column=DISAGGREGATION_COLUMN_START +
                            len(disaggregation_types) +
                            idx).value = ", ".join([str(dv.id) for dv in dvs])
            self.sheet.cell(row=3, column=DISAGGREGATION_COLUMN_START +
                            len(disaggregation_types) +
                            idx).value = " + ".join([disaggregation_values_by_type[dv.id] for dv in dvs])
            self.sheet.cell(row=4, column=DISAGGREGATION_COLUMN_START + len(disaggregation_types) +
                            idx).value = "#indicator+value+" + str("+".join([dv.value for dv in dvs]))
            disaggregation_values_map[", ".join(
                [str(dv.id) for dv in dvs])] = DISAGGREGATION_COLUMN_START + len(disaggregation_types) + idx
        ...
        self.sheet.cell(
            ...
            column=DISAGGREGATION_COLUMN_START +
            len(disaggregation_types) +
            len(disaggregation_values)).value = "Total"
        self.sheet.cell(
            row=1,
            column=DISAGGREGATION_COLUMN_START +
            len(disaggregation_types) +
            len(disaggregation_values)).alignment = Alignment(
            horizontal='center')
        disaggregation_values_map["()"] = DISAGGREGATION_COLUMN_START + \
            ...
            for location_data in locations_data.all():
                ...
                while(True):
                    ...
                    self.sheet.cell(
                        ...
                        column=20 +
                        level *
                        2).value = location.p_code
                    self.sheet.cell(
                        row=start_row_id,
                        column=20 +
                        level *
                        2 -
                        1).value = location.gateway.name
                    if location.parent:
