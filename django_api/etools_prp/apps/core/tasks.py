...
def process_period_reports():
    ...
    for pd in ProgrammeDocument.objects.filter(id=4482).order_by("id"):
        try:
...
def _process_pd_reports(pd):
    ...
    pd = ProgrammeDocument.objects.get(pk=pd.pk)
        ...
        if pd.document_type == PD_DOCUMENT_TYPE.GDD:
            for report_type in ["QPR"]:  # for now on
                ...
                if latest_progress_report:
                    ...
                    logger.info("No existing %s GPD ProgressReport found — creating first one", report_type)

                    default_start = pd.start_date or datetime.now().date()
                    ...
                    logger.info(f"Created default GPD ProgressReport for PD {pd.id} from {default_start} to {default_end}")
                    ...
                    if report_type == 'QPR':
                        generate_from_date_qpr = default_start
