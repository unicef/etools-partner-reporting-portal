...
class ProgressReport(TimeStampedModel):
    ...
    @property
    def gpd_addition_id(self):
        addition = self.gpd_addition_safe
        return addition.id if addition else None

    @property
    def gpd_addition_safe(self):
        if not hasattr(self, '_cached_gpd_addition'):
            try:
                self._cached_gpd_addition = self._meta.get_field('gpd_addition').related_model.objects.get(
                    progress_report=self)
            except GPDProgressReportAddition.DoesNotExist:
                if self.programme_document.document_type == PD_DOCUMENT_TYPE.GDD:
                    self._cached_gpd_addition = GPDProgressReportAddition.objects.create(progress_report=self)
                else:
                    self._cached_gpd_addition = None
        return self._cached_gpd_addition

    @property
    def is_gdd_type(self):
        return self.programme_document and self.programme_document.document_type == PD_DOCUMENT_TYPE.GDD
...
class GPDProgressReportAddition(models.Model):
    progress_report = models.OneToOneField(
        ProgressReport,
        on_delete=models.CASCADE,
        related_name='gpd_addition'
    )
    ...
    def __str__(self):
        return f"GPD Addition for Progress Report ID {self.progress_report.id}"
