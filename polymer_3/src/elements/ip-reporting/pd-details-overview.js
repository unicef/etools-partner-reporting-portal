...
class PdDetailsOverview extends UtilsMixin(LocalizeMixin(ReduxConnectedElement)) {
  ...
  `;
    ...
    _computeReportingRequirements(reportingPeriods) {
        if (!reportingPeriods) {
            return;
        }
    ...
    _computePdDetailsUrl(locationId, pdId) {
        if (!locationId || !pdId) {
            return;
        }
        return Endpoints.programmeDocumentDetail(locationId, pdId);
    }
    _displayFullName(types) {
        if (!types) {
            return '';
        }
        return types
    ...
    _getPdRecord() {
        ...
        this._pdDetailDebouncer = Debouncer.debounce(this._pdDetailDebouncer, timeOut.after(100), () => {
            const pdThunk = this.$.programmeDocumentDetail.thunk();
            // Cancel the pending request, if any
            this.$.programmeDocumentDetail.abort();
            pdThunk()
                .then((res) => {
                this.pd = res.data;
            })
...
], PdDetailsOverview.prototype, "pd", void 0);
__decorate([
    property({ type: Object })
], PdDetailsOverview.prototype, "amendmentTypes", void 0);
__decorate([
    property({ type: Boolean, computed: '_computeLoaded(pd)' })
], PdDetailsOverview.prototype, "loaded", void 0);
__decorate([
    property({ type: String, computed: 'getReduxStateValue(rootState.location.id)' })
], PdDetailsOverview.prototype, "locationId", void 0);
__decorate([
    property({ type: Object, computed: '_computeReportingRequirements(pd.reporting_periods)' })
], PdDetailsOverview.prototype, "reportingRequirements", void 0);
__decorate([
    property({ type: String, computed: 'getReduxStateValue(rootState.programmeDocuments.current)' })
], PdDetailsOverview.prototype, "pdId", void 0);
__decorate([
    property({ type: String, computed: '_computeProgrammeDocumentsUrl(locationId)' })
], PdDetailsOverview.prototype, "programmeDocumentsUrl", void 0);
__decorate([
    property({ type: String, computed: '_computePdDetailsUrl(locationId, pdId)' })
], PdDetailsOverview.prototype, "programmeDocumentDetailUrl", void 0);
