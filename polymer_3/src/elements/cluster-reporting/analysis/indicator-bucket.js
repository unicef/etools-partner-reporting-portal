...
class IndicatorBucket extends LocalizeMixin(ReduxConnectedElement) {
    ...
    _computePartnerProjectTitle(data) {
        if (data.type === 'partneractivityprojectcontext') {
    ...
    _handleOpenedChanged(e, data) {
        e.stopPropagation();
        if (data.value) {
            const indicatorDetails = e.target.querySelector('analysis-indicator-details');
            try {
                if (indicatorDetails) {
...
], IndicatorBucket.prototype, "data", void 0);
__decorate([
    property({ type: String, computed: '_computePrefix(data, localize)' })
], IndicatorBucket.prototype, "prefix", void 0);
