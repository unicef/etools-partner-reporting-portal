...
class AddActivityFromProjectModal extends LocalizeMixin(UtilsMixin(ModalMixin(ReduxConnectedElement))) {
  static get template() {
    return html`
      ...
      <etools-prp-ajax id="partnerActivities" params="[[activitiesParams]]" url="[[partnerActivitiesUrl]]">
      </etools-prp-ajax>
  ...
  static get observers() {
    return ['_fetchActivities(data.cluster.cluster)', '_fetchObjectives(data.custom.cluster)'];
  }
  ...
  _fetchActivities(clusterId: string) {
    ...
    this.set('partnerActivitiesUrl', Endpoints.partnerActivityList(this.responsePlanId) + '?cluster_id=' + clusterId);
    ...
    thunk()
      ...
      .then((res: any) => {
        ...
        this.set(
          'activities',
          this.activities.filter((item: any) => {
