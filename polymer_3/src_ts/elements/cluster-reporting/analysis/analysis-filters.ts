...
class AnalysisFilter extends UtilsMixin(ReduxConnectedElement) {
  static get template() {
    return html`
      ...
      <filter-list filters="{{filters}}" ignore="loc_type">
        <div class="app-grid">
          <cluster-filter-multi id="clusters" class="item" value="[[_withDefault(queryParams.clusters, '')]]">
          </cluster-filter-multi>

          <partner-type-filter-multi class="item" value="[[_withDefault(queryParams.partner_types, '')]]">
          ...
          <location-type-filter id="locationType" class="item" value="[[_withDefault(queryParams.loc_type, '')]]">
  ...
  static get observers() {
    return ['_resetNarrowLocType(queryParams.loc_type)'];
  }
  ...
  _resetClusterObjectives() {
    this._clusterObjectivesTypeDebouncer = Debouncer.debounce(
      this._clusterObjectivesTypeDebouncer,
      timeOut.after(100),
      () => {
  ...
  _resetLocations() {
    this._locationsDebouncer = Debouncer.debounce(this._locationsDebouncer, timeOut.after(100), () => {
      this.set('queryParams.locs', null);
    });
  }

  _resetNarrowLocType(locType: any) {
    this._narrowLocTypeDebouncer = Debouncer.debounce(this._narrowLocTypeDebouncer, timeOut.after(100), () => {
