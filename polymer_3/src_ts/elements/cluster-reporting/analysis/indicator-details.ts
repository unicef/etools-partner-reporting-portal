...
export class IndicatorDetails extends LocalizeMixin(UtilsMixin(ReduxConnectedElement)) {
  static get template() {
    return html`
      ...
      <etools-prp-ajax id="indicator" url="[[indicatorUrl]]"> </etools-prp-ajax>
      ...
      <template is="dom-if" if="[[!loading]]" restamp="true">
        <div class="header layout horizontal justified">
          <div class="col flex-none">
            ...
            <dl class="indicator-data layout horizontal">
              ...
              <dd>
                <frequency-of-reporting type="[[data.frequency]]"> </frequency-of-reporting>
              </dd>
            </dl>
            <dl class="indicator-data layout horizontal">
        ...
        <div class="app-grid">
          <partners-by-status class="item" data="[[data.partners_by_status]]"> </partners-by-status>
        ...
        <div class="app-grid">
          <current-progress-by-partner class="item" data="[[data.current_progress_by_partner]]">
          </current-progress-by-partner>

          <current-progress-by-location class="item" data="[[data.current_progress_by_location]]">
          </current-progress-by-location>
        </div>

        <hr />

        <template is="dom-if" if="[[_equals(data.indicator_type, 'Partner Activity Indicator')]]">
          <div class="app-grid">
  ...
  init() {
    ...
    this.reduxStore
      .dispatch(analysis_indicators_fetchSingle(indicatorThunk, this.indicatorId))
      // @ts-ignore
      .catch((_err: any) => {
        // TODO: error handling
      });
