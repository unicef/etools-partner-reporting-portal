...
(function() {
  ...
  class EtoolsPrpPermissions extends UtilsMixin(ReduxConnectedElement) {
    ...
    _computeImoClusters(profile: GenericObject) {
      return profile.prp_roles ?
        profile.prp_roles.filter(function(item: any) {
    ...
    _computeParams(prpRoles: any[], imoClusters: any[], partner: GenericObject, workspace: string, responsePlan: GenericObject) {
      return {
        prpRoles: prpRoles,
